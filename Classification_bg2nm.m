...
for f_no=3:y1
    ...
    for num=1:numberOfPose       
        ...
        for ff_no=33:y2
            ...
            for fff_no=2+(num):numberOfPose:numberOfPose*2+2%y3
...
path1
str="bg to nm where considered as test case"
ans1 = (accu/num1)*100;
ans1
accu1 = [accu1,ans1];
...
% path = 'D:\sanjay_server_data\Code_For_GAN6\CO2SI_GEI_csv\';
% csvwrite(char(strcat(path,'PEI_CASIA_bg_to_nmCV.csv')),cv111);
% csvwrite(char(strcat(path,'PEI_CASIA_bg_to_nmPV.csv')),pv111);
% csvwrite(char(strcat(path,'PEI_CASIA_bg_to_nmAccu.csv')),accu1);