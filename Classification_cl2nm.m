...
for f_no=3:y1
    ...
    for num=1:numberOfPose       
        ...
        for ff_no=33:y2
            ...
            for fff_no=numberOfPose*2+2+(num):numberOfPose:numberOfPose*4+2%y3
...
path1
str="cl to nm where considered as test case"
ans1 = (accu/num1)*100;
ans1
accu1 = [accu1,ans1];
...
% path = 'H:\Neurocomputing_COFEI\Classify_Cyclic_PEI_csv\';
% csvwrite(char(strcat(path,'PEI_CASIA_C_fq_to_nmCV.csv')),cv111);
% csvwrite(char(strcat(path,'PEI_CASIA_C_fq_to_nmPV.csv')),pv111);
% csvwrite(char(strcat(path,'PEI_CASIA_C_fq_to_nmAccu.csv')),accu1);